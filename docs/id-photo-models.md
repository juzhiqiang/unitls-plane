...
8. **EP 与优化级别按量化方式(高精度报错的真正根因)**:RMBG-2.0 `q4f16` 有两个独立问题,必须同时处理,缺一仍会"处理失败":
   - **EP**:含 `MatMulNBits` 算子(实测 198 个),onnxruntime-web 的 WebGPU EP 不支持;即便列 `['webgpu','wasm']` 也会在创建/推理时直接抛错而非按算子回退。故 `q4f16` 强制 `['wasm']`。
   - **优化级别**:在 `graphOptimizationLevel 'all'` 下,session 初始化阶段 `SimplifiedLayerNormFusion` 图融合报错(`InsertedPrecisionFreeCast_.../patch_embed/norm/Constant_output_0` 名缺失),与 EP 无关(图融合先于 EP 分配)。实测降到 `'basic'` 即可绕过,数值结果不变(融合仅用于加速)。`fp16` 在 `'all'` 下正常(实测),保留加速。

   `handleInit` 据 `quant` 决策:`q4f16` → `['wasm']` + `opt 'basic'`;`fp16` → `['webgpu','wasm']` + `opt 'all'`,且 WebGPU 整图创建失败时再退回纯 `['wasm']`。`ModelMeta.quant` 经 init 消息传入 worker;`ready.ep` 仍上报 WebGPU 可用性(用于前端高精度开关),与 session 实际后端解耦。验证脚本见 `artifacts/verify-q4f16-{cpu,optlevel}.cjs`(本地不入库)。
