...
- AI 生图切换历史会话更快:历史图片改为先加载小尺寸缩略图并按需懒加载,点开或下载时才取原图,不再一次性把整个会话的原图全部下载,切会话几乎即时出图。
- 页面上可点击的按钮在悬停时统一显示手势光标,交互指向更清晰。
- 首页文案改版:首屏直接说明产品能做什么(图片、PDF、文档、字体一站式在线处理),更容易快速了解各项工具。
- 精简图片工具列表:移除与「图片压缩」重复的「批量图片输出」「前后对比预览」入口,压缩工具本身已支持批量打包与前后对比。

### 发布

- 发布
  `v0.10.1`:以 v0.10.0 之后的体验改进为主的补丁版本(AI 生图历史图片加载优化、手势光标、首页文案、图片工具列表精简);根包、Web、API 与所有共享包统一升级到
  `0.10.1`,页面更新日志中英文 `PublicSite.changelog` 已新增对应用户侧条目。
