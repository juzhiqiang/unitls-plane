...
- 旧 page/limit API 继续兼容；文件、回收站、任务页面已使用 cursor 和
  `includeTotal=false`，跳过深层 offset 与 `COUNT(*)`，返回
  `total: null`。页面提供首页、上一页、下一页和当前页号，不提供任意页码跳转或总页数；筛选/账号改变时重置，文件删除/恢复/清空成功后回首页。API 缺省 includeTotal 仍为 true。并发新增/删除时不提供快照一致性。
- 任务列表支持 `category=image|pdf|font` 服务端类别筛选，并继续兼容精确 `type`
  筛选。类别会映射到完整任务类型集合，且写入游标 scope，避免不同类别复用游标；任务页筛选不再只过滤当前页。
- Web 私有 React
...
- 生图失败时，服务端把上游真实报错经 `apps/api/src/modules/tasks/services/image-error-sanitizer.ts`
  ...
  `tasks.errorMessage`；意外错误同样脱敏后透出，给不出内容时回退固定文案。前端按固定模板反解并本地化成母语原因句子，HTTP状态码只用于分类、不展示给用户；认不出的原因原样展示。内容策略拒绝时，上游给出的具体原文（如「抱歉，我不能帮助生成…」）直接展示，没有原文才用本地化通用文案。内容策略标记表含阿里云绿网措辞（`green net`、`inappropriate content`、`data_inspection_failed`）与中文模型/网关的拒绝措辞（「抱歉，我不能」「色情」「违规」等），`error`
  字段为纯字符串的报错体也能提取为原因。
