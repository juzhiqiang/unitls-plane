...
import { countTasksCreatedToday } from '../../../src/modules/tasks/daily-task-quota';
...
// 同一次 `bun test ./test` 里,files.service.test.ts / account.repository.test.ts 等文件会把
...
const { countTasksCreatedToday } = await import(${JSON.stringify(
  new URL('../../../src/modules/tasks/daily-task-quota.ts', import.meta.url)
    .href
)});
const conditions = [];
