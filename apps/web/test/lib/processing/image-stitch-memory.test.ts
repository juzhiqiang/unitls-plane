...
vi.mock('@/lib/processing/image-bitmap', () => ({ decodeImage: mocks.decode }));
vi.mock('@/lib/processing/canvas-surface', () => ({
  createSurface: () => ({
    ctx: { drawImage: mocks.draw, fillRect: vi.fn() },
    toBlob: mocks.encode,
  }),
}));
import { renderStitch } from '@/lib/processing/image-stitch-client';
const options = {
  width: 100,
  gap: 0,
