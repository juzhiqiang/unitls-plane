...
export interface GeneratedImageMarkerOptions {
  model: string;
  /** 实际出图的来源 id(多来源容错路由下,同一模型可能来自不同网关)。 */
  source?: string;
  generatedAt: Date;
}

/**
 * 给 AI 生成图写入隐式来源标识(生成方、模型、来源、生成时间)。
 *
...
export async function markGeneratedImage(
  input: Buffer,
  { model, source, generatedAt }: GeneratedImageMarkerOptions
): Promise<Buffer> {
  return sharp(input)
    .withExif({
      IFD0: {
        Software: 'Utils-Plane AI Image Generation',
        ImageDescription: `AI-generated image; model=${model};${source ? ` source=${source};` : ''} generatedAt=${generatedAt.toISOString()}`,
      },
    })
    .png()
    .toBuffer();
